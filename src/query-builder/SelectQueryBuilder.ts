        ...
        if (joinAttributeMetadata) {
            if (
                (
                    ...
                    joinAttributeMetadata.deleteBooleanColumn
                ) &&
                !this.expressionMap.withDeleted
            ) {
                const conditionDeleteColumn =
                    joinAttributeMetadata.deleteDateColumn ?
                        `${aliasName}.${joinAttributeMetadata.deleteDateColumn.propertyName} IS NULL` :
                        `${aliasName}.${joinAttributeMetadata.deleteBooleanColumn!.propertyName} = false`;
